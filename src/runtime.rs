...
use crate::loader::MockModuleLoader;

pub const HEAP_INIT: usize = 1 << 10;
pub const SNAPSHOT: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/snapshot.bin"));
pub const ALIGNED_SNAPSHOT_SIZE: usize = SNAPSHOT.len().next_power_of_two();
...
impl Runtime {
    ...
    pub fn new(location: Url, depth: u8) -> Result<Self> {
        ...
        let mut deno = JsRuntime::new(RuntimeOptions {
            ...
            module_loader: Some(Rc::new(MockModuleLoader::new())),
...

        {
            ...
            std::thread::spawn(move || {
                while oom_rx.recv().is_ok() {
                    safe_handle.terminate_execution();
                    println!("called terminate_execution");
                }
            });
